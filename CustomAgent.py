...
class MultipleTopologyAgent(BaseAgent):
    ...
    def _build(self):
        ...
        combined_lines = self.combined_action_keys(action_dic=self.line_actions_dic,
                                                   depth=self.max_simultaneous_sub_actions)
        ...
        combined_subs = self.combined_action_keys(action_dic=self.sub_actions_dic,
                                                  depth=self.max_simultaneous_sub_actions)
        ...
        for sub_comb in combined_subs:
            sub_comb_acts = self.merge_combined_actions_subs(self.sub_actions_dic, sub_comb)
    ...
    def unitary_sub_actions(self):
        ...
        for sub_id in sub_to_switch:
            ...
            action_def_1={"set_bus": {"substations_id": [(sub_id, target_topology_1)] } }
 
            target_topology_2= np.ones(n_elements)
            target_topology_2[[2*i for i in range(int(np.round(n_elements/2)))]] = 2
            action_def_2={"set_bus": {"substations_id": [(sub_id, target_topology_2)] } }
