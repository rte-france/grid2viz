...
def consumption_profiles(episode, freq="30T"):
    ...
    #filter intercos
    idx_no_interco = [i for i in range(len(episode.load.equipment_name)) if 'interco' not in episode.load.equipment_name[i]]

    load = pd.pivot_table(
        episode.load.loc[idx_no_interco], index="timestamp", columns=["equipment_name"], values="value"
    ).sum(axis=1).resample(freq).mean().to_frame(name="load").reset_index()

    if freq == "H":
        load["timestamp"] = load.timestamp.dt.hour
    elif "T" in freq:
        load["timestamp"] = load.timestamp.dt.time.map(
            lambda x: x.strftime("%H:%M")
        )
    else:
        raise ValueError("Only hourly (\"H\") or minutely (\"xT\") freqs are "
                         f"supported. Passed: {freq}")

    daily_load_distrib = load.groupby(["timestamp"]).aggregate(
